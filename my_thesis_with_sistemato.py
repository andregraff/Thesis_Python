...
def plot_overlapping_line_charts(df_list, x_col, y_col, labels=None, x_label=None, y_label=None, title=None, save_path=None, zoom_month=None, fontsize=None):
    """
    ...
    # Format x-axis to display all months or zoom to the specified month
    if zoom_month:
        ax.set_xlim(pd.Timestamp(zoom_month), pd.Timestamp(zoom_month) + pd.DateOffset(months=1))
        # Dynamically adjust y-axis scale to match the data range in the zoomed month
        data_in_zoomed_month = df_list[0][(df_list[0][x_col] >= pd.Timestamp(zoom_month)) & (df_list[0][x_col] < pd.Timestamp(zoom_month) + pd.DateOffset(months=1))]
        ax.set_ylim(data_in_zoomed_month[y_col].min(), data_in_zoomed_month[y_col].max())
...
# # Example usage
# eso_path1 = r"#enplus_models\Models_8.5_Originals\OFFICE\DG\B2_O_L1_DG_E_0.2_0.8_TS2\B2_R_L2_S_2windows.idf.eso"
# key_number = 549
# df1 = df_kpi_in_esofile(eso_path1, key_number)

# eso_path2 = r"E:\Week1\Thesis_Python-main\#enplus_models\Models_8.7_fixed\OFFICE\DG\B2_O_L1_DG_E_0.2_0.8_TS2\eplusout.eso"
# key_number = 549
...
#                               x_col="Date",
#                               y_col="Value",
#                               labels=["Heating (v8.5)","Heating (v8.7)"],
#                               x_label="Date",
#                               y_label="Sensible Heating Rate ZONAGIORNO [W]",
#                               title="Zone Ideal Loads Zone Sensible Heating Rate [W]",
#                               save_path= "comp_B2_O_L1_DG_E_0.2_0.8_TS2_sensib_heat.png",
#                               zoom_month="Mar 2009",
#                               fontsize = 22)
...
def main(run_files=False, collect_kpi=False, verify = True):
    ...
    df_comp85_87['DeltaAC%(v8.5-8.7)'] = round((df_final['AC(v8.5)'] - df_final['AC(v8.7)'])/ df_final['AC(v8.7)'] *100, 2)    # Calculate the %diff   
    # Annual Heating
    df_comp85_87['DeltaAH%(v8.5-8.7)'] = round((df_final['AH(v8.5)'] - df_final['AH(v8.7)'])/ df_final['AH(v8.7)'] *100, 2)    # Calculate the %diff
    # Annual Total
    df_comp85_87['DeltaAT%(v8.5-8.7)'] = round((df_final['AT(v8.5)'] - df_final['AT(v8.7)'])/ df_final['AT(v8.7)'] *100, 2)    # Calculate the %diff
    ...
    create_heamap1(df_comp85_87, 5, 100, annot_rot=0, title="Delta% from IDFs 8.5 to 8.7", savepath=folder_images, palette="coolwarm")    
    ...
    def highlight_differences(row):
        ...
        backgrounds = [''] * len(row)  # Inizializza una lista vuota di stili per tutte le colonne

        for col in columns_to_check:
            if row[col] != row['ID_code(8.7)']:  # Confronta con Colonna1
                color = 'red'
            else:
                color = 'green'
            
            # Assegna lo stile solo alle colonne di interesse
            col_index = df.columns.get_loc(col)
            backgrounds[col_index] = 'background-color: {}'.format(color)
    ...
    return df_id_verified_85, df_id_verified_87, df_all_versions, df_topAH_comp, df_topAT_comp, results_dfs, rank_range, df_verifyIDF, df_comp85_87, df_comp87_222
    


if __name__  == "__main__" :
   df_id_verified_85, df_id_verified_87, df_all_versions, df_topAH_comp, df_topAT_comp, results_dfs, rank_range, df_verifyIDF, df_comp85_87, df_comp87_222 = main()    